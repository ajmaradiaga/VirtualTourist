...
class PhotoAlbumViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, NSFetchedResultsControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if displayLocation.photos.count == 0 {
            displayLocation.fetchForPhotos({ (success, errorString) -> Void in
                    if success {
                        println("Success fetching for photos.")
                    } else {
                        if errorString != nil {
    ...
    func setMapLocation () {
        ...
        let miles = 30.0;
        var scalingFactor = abs((cos(2 * M_PI * locationCoordinate.latitude / 360.0) ))
        
        var span = MKCoordinateSpan(latitudeDelta: miles/69.0, longitudeDelta: miles/(scalingFactor*69.0))
        
        var region = MKCoordinateRegion(center: locationCoordinate, span: span)
    ...
    @IBAction func retrieveNewCollectionOfPhotos(sender: AnyObject) {
        ...
        for index in 1...photosCollectionView.numberOfItemsInSection(0) {
            var cell = photosCollectionView.cellForItemAtIndexPath(NSIndexPath(forRow: index - 1, inSection: 0))! as! PhotoCollectionViewCell
        ...
        displayLocation.fetchForPhotos { (success, errorString) -> Void in
            if success {
                println("Successfully fetch for Photos")
            } else {
                if errorString != nil {
                    self.alertVC = Helper.raiseInformationalAlert(inViewController: self, withTitle: "Error", message: errorString!, completionHandler: { (alertAction) -> Void in
                        self.alertVC!.dismissViewControllerAnimated(true, completion: nil)
